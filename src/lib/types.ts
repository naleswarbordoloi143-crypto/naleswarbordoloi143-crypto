...
export type NfcEntityType = 'FARMER' | 'HARVEST' | 'MACHINERY';
export type NfcTagStatus = 'ACTIVE' | 'INACTIVE' | 'LOST' | 'BLOCKED';
export type NfcScanResult = 'SUCCESS' | 'NOT_FOUND' | 'UNAUTHORIZED' | 'BLOCKED' | 'ERROR';

export interface NfcTag {
  id: string;
  tag_uid: string;
  entity_type: NfcEntityType;
...
export type TraceabilityEventType =
  | 'HARVEST_CREATED' | 'NFC_ASSIGNED' | 'COLLECTION_SCANNED'
  | 'QUALITY_CHECKED' | 'BUYER_VIEWED' | 'ORDER_CONFIRMED' | 'COLLECTION_COMPLETED';

export interface NfcTraceabilityEvent {
  id: string;
  lot_id: string;
  tag_id: string | null;
...
export interface NfcFarmerEntity {
  type: 'FARMER';
  farmerId: string;
  name: string;
  village: string;
  ...
  cluster: string | null;
  clusterCrop: string | null;
  farmCrops: { crop_name: string; status: string; area_acres: number; expected_yield_kg: number }[];
  harvestLots: { crop_name: string; quantity_kg: number; quality_grade: string; harvest_date: string | null; status: string; price_per_kg: number; location: string }[];
  machinery: { name: string; type: string; price_per_hour: number; available: boolean; location: string }[];
  totalContributionKg: number;
  currentCrop: string | null;
  cropStatus: string | null;
}

export interface NfcHarvestEntity {
  type: 'HARVEST';
  lotId: string;
  crop: string;
  quantity: number;
  ...
  contributions?: { farmerName: string; quantityKg: number }[];
}

export interface NfcMachineryEntity {
  type: 'MACHINERY';
  machineryId: string;
  name: string;
  machineType: string;
...
export type NfcEntity = NfcFarmerEntity | NfcHarvestEntity | NfcMachineryEntity;
